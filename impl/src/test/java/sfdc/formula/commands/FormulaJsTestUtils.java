...
import sfdc.formula.*;
import sfdc.formula.impl.FormulaTypeUtils;
...
public class FormulaJsTestUtils {
    ...
    public static Object evaluateFormula(Formula formula, FormulaDataType columnType, FormulaContext context,
    	...
    	if (IS_GRAAL) {
            ...
            try {
                ...
                if (jsMap != null) {
                    FormulaTypeUtils.convertToNashorn(engine, jsMap, context);
                ...
                if (globalMap != null) {
                    globalMap.entrySet().stream().forEach((e) -> engine.getBindings(ScriptContext.GLOBAL_SCOPE)
                            .put(e.getKey(), FormulaTypeUtils.convertToNashorn(engine, e.getValue(), context)));
                }
    ...
    public static ScriptEngine getNashornEngine() {
        ...
        if (bindings == null) {
            ...
            try {
                ...
                ResourceFolder folder = ResourceFolder.create(FormulaJsTestUtils.class.getClassLoader(), "sfdc/formula",
                ...
                for (Method m : Require.class.getDeclaredMethods()) {
                    if (m.getParameterCount() == 3) {
                        assert m.getName().equals("enable"); // Avoid referencing NashornScriptEngine directly to avoid
                                                             // "restriction" error
                        try {
    ...
    public static Object convertResultFromNashorn(Object result, ScriptEngine engine, FormulaDataType columnType) {
        try {
            result = FormulaTypeUtils.convertFromNashorn(engine, result);
            ...
            result = FormulaTypeUtils.convertFromNashorn(FormulaJsTestUtils.getBindingsEngineForUnwrappingStuff(),
                    result);
        }
    ...
    public static Object convertResultFromGraal(Object result, FormulaDataType columnType) {
        ...
        try {
            result = FormulaTypeUtils.convertFromGraal(result, columnType);
        } catch (IllegalArgumentException ex) {
            throw ex;
        }
        if (columnType.isDateTime() && result instanceof Date) {
    ...
    public static ScriptEngine getGraalEngine() {
        ...
        if (bindings == null) {
            ...
            try {
                ...
                // URL decimalUrl = FormulaJsTestUtils.class.getClassLoader().getResource("sfdc/formula/decimal.js");
                // Object decObj = compEngine.eval("load('"+new File(decimalUrl.toURI()).getAbsolutePath()+"')");
                try (Reader decimal = new InputStreamReader(
                        FormulaJsTestUtils.class.getClassLoader().getResourceAsStream("sfdc/formula/decimal.js"))) {
                    Bindings engineBindings = compEngine.getBindings(ScriptContext.ENGINE_SCOPE);
