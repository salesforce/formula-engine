...
import sfdc.formula.impl.BaseFormulaGenericTests.FormulaTestRunnable;
...
public class FormulaTestCaseInfo {
    ...
    private Set<EvaluationContext> parseContext(String contexts) {
        ...
        if (contexts.equals("none")) {
            return ctx;
        }
        if (contexts.contains("formula")) {
           ctx.add(EvaluationContext.Formula);
        }
        if (contexts.contains("template")) {
            ctx.add(EvaluationContext.Template);
        }
        if (contexts.contains("visualforce")) {
            ctx.add(EvaluationContext.Visualforce);
        }
        if (contexts.contains("workflow")) {
    ...
    public String getDataFileName() {
        return this.dataFileName.equals("") ? null : this.dataFileName;
    }
    ...
    private List<FormulaTestRunnable> generateTypeSwappedTCs(int swappableFieldCount) throws CloneNotSupportedException {
        ...
        for (List perm : FormulaTestUtils.getSwapSets(swappableFieldCount)) {
            ...
            if (!multipleResultTypes) {
                ...
                if (!resultType.equals(testCaseFieldInfo.getReturnType())) {
                    continue;
                }
            }
    ...
    //
    private List<FieldDefinitionInfo> createList = new LinkedList<FieldDefinitionInfo>();
