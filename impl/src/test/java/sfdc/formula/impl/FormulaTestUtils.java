...
public class FormulaTestUtils {
    ...
    public static List<FormulaTestCaseInfo> getTestCases(String xmlFileName, Predicate<FormulaTestCaseInfo> filter, String owner,
        ...
        for (int tcCount = 0; tcCount < tcList.getLength(); tcCount++) {
            ...
            if (tcNegative) {
                errorCode = testCase.getAttribute("errorcode");
                errorMsg = testCase.getAttribute("errmsg");
    ...
    public static List<List<String>> getDataFromFile(String fileName) throws IOException, FileNotFoundException {
        ...
        BufferedReader input = new BufferedReader(new InputStreamReader(FormulaTestUtils.class.getResourceAsStream("/sfdc/formula/impl/data/"
            + fileName), "UTF-8"));
        String temp;
    ...
    private static List<FieldDefinitionInfo> extractFieldsDefintions(NodeList fieldList, String entityName)
        ...
        for (int nField = 0; nField < fieldList.getLength(); nField++) {
            ...
            boolean isStandard = field.getAttribute("standardfield").equalsIgnoreCase("true");
