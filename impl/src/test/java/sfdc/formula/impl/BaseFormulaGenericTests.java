...
import sfdc.formula.*;
import sfdc.formula.impl.BaseFieldReferenceTest.FieldTestFormulaValidationHooks;
import sfdc.formula.impl.FormulaTestCaseInfo.CompareType;
import sfdc.formula.impl.MapFormulaContext.MapEntity;
import sfdc.formula.impl.MapFormulaContext.MapFieldInfo;
...
abstract public class BaseFormulaGenericTests extends TestSuite {
	...
	IOException {
		super(owner);
		createTestCases(this, "/sfdc/formula/impl/formulatests.xml", "Positive Tests", true, owner, testLabelsAttribute, swapResultTypes);
	}
	...
	abstract static class BaseFormulaGenericTest extends ParserTestBase {
		MapEntity mapEntity;
		Map<String,Map<String,Object>> objects = new HashMap<>();

		public BaseFormulaGenericTest(FormulaTestCaseInfo testCase, String name, String entity, boolean positive, BaseFormulaGenericTests suite) {
			super(name, false);
			...
			try {
				...
				if (fieldList.size() > 0) {
					for (List<String> testDataRow : testData) {
						...
						setDataForRec(fieldList, testDataRow, fieldInfo.getUnMaskedEntityName(), entityRecId, jestDataModel);
		...
		private String getTestCaseResults(
			...
			for (String key: getKeyNames()) {
				...
				String resultForKey = null;
				if (results.containsKey(key)) {
					resultForKey = results.get(key);
				} else {
					resultForKey = "not-tested";
				}
				out.println(String.format("         <%s>%s</%s>",
						key, FormulaTextUtil.escapeToXml(resultForKey), key));

				if(key.equals("formula")) {
					if (!jestDataModel.tests.isEmpty()) {
		...
		private void setDataForRec(List<FieldDefinitionInfo> fieldList, List<String> testDataRow, String entityName,
			...
			for (int i = 0; i < fieldList.size(); i++) {
				fieldName = fieldList.get(i).getReferenceName();
