...
import sfdc.formula.parser.gen4.*;
...
public class FormulaUtils {
    ...
    private static FormulaAST parseTemplateWithANTLR4(String source, FormulaProperties properties) throws FormulaException {
        ...
        if(inFormula) { //never closed embedded formula
            CommonToken eofToken = new CommonToken(SfdcFormulaTokenTypes.EOF, null);
            eofToken.setLine(1);
            ...
            if(inComment) {
                ...
                e = new MismatchedTokenException(null, eofToken, SfdcFormulaTokenTypes.END_EMBEDDED_FORMULA, false, null);
            }
    ...
    private static MismatchedCharException createMismatchCharException(char found, char expecting, int line, int column) {
        MismatchedCharException e = new MismatchedCharException();
        e.foundChar = found;
        e.expecting = expecting;
        e.mismatchType = MismatchedCharException.CHAR;
        e.line = line;
        e.column = column;
        return e;
    }
    ...
    private static FormulaAST parseFormulaWithANTLR4(String source, FormulaProperties properties, int startIndex) throws FormulaException {
        if(source == null || source.length() == 0) {
            throw new FormulaParseException(new ANTLRException(new NullPointerException()));
        }
    ...
    public static void compareTokens(Token t1, Token t2) throws Exception {
        if(t1 == null && t2 == null) {
            return;
        }
